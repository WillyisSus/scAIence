...
export const TrackRow = ({ tasks, id, onResizeItem, handleDeleteContextMenu}) => {
...
return (
    <SortableContext id={id} items={tasks} strategy={horizontalListSortingStrategy}>
        <div ref={setNodeRef} className="flex flex-1 flex-row h-16 relative min-w-[800px] w-full bg-gray-50">
            {tasks.map((task) => (
                <TrackRowItem handleDeleteContextMenu={handleDeleteContextMenu} onResizeStop={onResizeItem} key={task.id} id={task.id} item={task} />
            ))}
        </div>
    </SortableContext>
);
